...
#set document(
  ...
  keywords: ("machine learning systems", "overfitting", "CNN", "RNN", "deep learning"),
)
...
=== Answer
In a real-world e-commerce ML system, deploying a model is only the first step. The bigger challenge is keeping the model reliable, fast, and useful after the business environment starts changing. Product ML is difficult because features like user behaviour, products, prices, stock, campaigns, and fraud patterns can shift faster than the original training dataset.
...
All in all, a reliable production ML system is not bound to just the model or the pipeline. Data scientists can improve the model, but engineers still have to serve it reliably, and operations teams provide the reality when the system creates business-side problems. MLOps dashboards, retraining jobs, and LLM-based log analysis are useful support tools. They make failures easier to detect and debug, but they do not make the system safe by themselves.
